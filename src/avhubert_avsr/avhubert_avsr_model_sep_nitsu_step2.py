from src.nets.backend.e2e_asr_avhubert_sep_nitsu_step2 import E2E
...
class AVHubertAVSROutput(ModelOutput):
    ...
    loss_sep: Optional[torch.FloatTensor] = None
    acc: Optional[torch.FloatTensor] = None
    sisnr_s_main_i: Optional[torch.FloatTensor] = None
...
class AVHubertAVSR(PreTrainedModel):
    ...
    def forward(
        ...
        # loss, loss_ctc, loss_att, acc = self.avsr(videos, audios, video_lengths, audio_lengths, labels, label_audios, label_noises, label_audio_lengths, label_noise_lengths)
        loss, loss_ctc, loss_att, loss_sep, acc, sisnr_s_main_i = self.avsr(
            videos,
            audios,
            video_lengths,
            audio_lengths,
            ...
            label_audios_raw=label_audios_raw,
            label_noises_raw=label_noises_raw,
        )
