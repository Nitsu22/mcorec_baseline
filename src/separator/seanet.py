...
class seanet(nn.Module):
    def __init__(self, N = 256, L = 40, B = 64, H = 128, K = 100, R = 3):
        '''
    ...
    def forward(self, mixture, visual, M = 1):
        ...
        est_speech, est_noise = self.separator(mixture_w, visual, M)
...
class seanet_separator(nn.Module):
    def __init__(self, N = 256, L = 104, B = 64, H = 128, K = 100, R = 3):
        '''
        Module list: Encoder - Decoder - Extractor
        B=1024 --> from Wav2Vec
        '''
        super(seanet_separator, self).__init__()
        self.N, self.L, self.B, self.H, self.K, self.R = N, L, B, H, K, R
        
        self.encoder   = Encoder(N)
        self.separator = Extractor_RNNs(N, L, B, H, K, R)
        self.decoder   = Decoder(N, L)
    ...
    def forward(self, mixture_feat, M = 1):
        # mixture_feat: B x T x 1024
        # print('mixture_feat', mixture_feat.shape)
        mixture_feat = torch.transpose(mixture_feat, 1, 2) # B x 1024 x T
        # print('mixture_feat', mixture_feat.shape)
        mixture_w = self.encoder(mixture_feat) # out: B x 256 x T
        # print('mixture_w', mixture_w.shape)
        # print('mixture_w after encoder', mixture_w)
        est_speech, est_noise = self.separator(mixture_w, M)
        mixture_w   = mixture_w.repeat(self.R, 1, 1)
        est_speech  = torch.cat((est_speech), dim = 0)
        est_noise   = torch.cat((est_noise), dim = 0)

        # print('est_speech', est_speech.shape)
        # mixture_feat = torch.transpose(mixture_feat, 1, 2) 
        est_speech  = self.decoder(mixture_w, est_speech)
        est_noise   = self.decoder(mixture_w, est_noise)

        est_speech = torch.transpose(est_speech, 1, 2)
        est_noise = torch.transpose(est_noise, 1, 2)
        # T_ori = mixture.size(-1)
        # T_res = est_speech.size(-1)
        # est_speech = F.pad(est_speech, (0, T_ori - T_res))
        # est_noise  = F.pad(est_noise, (0, T_ori - T_res))
...
class Extractor_RNNs(nn.Module):
    ...
    def forward(self, x, M):
        # TODO: FIND OUT THE DIMENSION OF x, x is the feature from AVHubert 
        # x size: Batch size x 400 x 1024 -> what is 400? T
        # x = torch.transpose(x, 1, 2)
        M, N, P = x.size() # M: batch size?, N?, P?
...
class Encoder(nn.Module):
    def __init__(self, N):
        ...
        self.N = N
        self.layer_norm = nn.GroupNorm(1, 1024, eps=1e-8)
        # self.conv1d_U = nn.Conv1d(1024, N, kernel_size=L, stride=L // 2, bias=False)
        self.conv1d_U = nn.Conv1d(1024, N, kernel_size=1, bias=False).float()

    def forward(self, mixture):
        # print('mixture input', mixture.shape)
        # mixture = torch.unsqueeze(mixture, 1)      # B, 1, L  = [2, 1, 64000]
        # mixture_w = F.relu(self.conv1d_U(mixture)) # B, D, L' = [2, 256, 3199]
        with torch.cuda.amp.autocast(False):
            mixture_w = mixture.to(torch.float32)
            # print("feat finite:", torch.isfinite(mixture_w).all())
            mixture_w = self.layer_norm(mixture_w)
            # print("after LN finite:", torch.isfinite(mixture_w).all())
            
            w = self.conv1d_U.weight
            # print("weight finite:", torch.isfinite(w).all())
            # print('mixture_w norm', mixture_w)
            mixture_w = self.conv1d_U(mixture_w) # B, D, L' = [2, 256, 3199]
        # print('mixture_w output', mixture_w)
        return mixture_w
...
class Decoder(nn.Module):
    ...
    def forward(self, mixture_w, est_mask):
        est_source = mixture_w * est_mask  # [M,  N, K] B * D * T
        est_source = torch.transpose(est_source, 2, 1) # [M,  K, N] B * T * D
        est_source = self.basis_signals(est_source)  # [M,  K, L] B * T * L
