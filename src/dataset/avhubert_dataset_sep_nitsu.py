...
class AudioTransform:
    ...
    def __call__(self, sample, label_sample=None, return_raw=False):
        # sample: T x 1
        # rtype: T x 1
        ...
        if (label_sample is not None):
            ...
            audio_fbank = self.audio_pipeline_fbank(audio)
            label_fbank = self.audio_pipeline_fbank(label_sample)
            noise_fbank = self.audio_pipeline_fbank(noise)
            if return_raw:
                audio_raw = normalize_audio(audio).squeeze(-1)
                label_raw = normalize_audio(label_sample).squeeze(-1)
                noise_raw = normalize_audio(noise).squeeze(-1)
                return audio_fbank, label_fbank, noise_fbank, audio_raw, label_raw, noise_raw
            return audio_fbank, label_fbank, noise_fbank
        else:
...
class DataCollator:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        for feature in features:
            ...
            audio_transform_outputs = self.audio_transform(label_audio, label_sample=label_audio, return_raw=True)
            if len(audio_transform_outputs) == 6:
                audio, label_audio, label_noise, audio_raw, label_audio_raw, label_noise_raw = audio_transform_outputs
            elif len(audio_transform_outputs) == 3:
                audio, label_audio, label_noise = audio_transform_outputs
                audio_raw = None
                label_audio_raw = None
                label_noise_raw = None
            else:
                label_noise = None
                audio_raw = None
                label_audio_raw = None
            ...
            if "label" in feature:
                ...
                if label_noise is None:
                    ...
                    sample = {"video": video, "audio": audio, "label": label, "label_audio": label_audio, "label_noise": label_noise}
                    if audio_raw is not None:
                        sample["audio_raw"] = audio_raw
                        sample["label_audio_raw"] = label_audio_raw
                        sample["label_noise_raw"] = label_noise_raw
        ...
        if 'audio_raws' in batch:
            batch['audios_raw'] = batch.pop('audio_raws')
            if batch['audios_raw'].dim() == 3 and batch['audios_raw'].size(1) == 1:
                batch['audios_raw'] = batch['audios_raw'].squeeze(1)
            batch.pop('audio_raw_lengths', None)
        if 'label_audio_raws' in batch:
            batch['label_audios_raw'] = batch.pop('label_audio_raws')
            if batch['label_audios_raw'].dim() == 3 and batch['label_audios_raw'].size(1) == 1:
                batch['label_audios_raw'] = batch['label_audios_raw'].squeeze(1)
            batch.pop('label_audio_raw_lengths', None)
        if 'label_noise_raws' in batch:
            batch['label_noises_raw'] = batch.pop('label_noise_raws')
            if batch['label_noises_raw'].dim() == 3 and batch['label_noises_raw'].size(1) == 1:
                batch['label_noises_raw'] = batch['label_noises_raw'].squeeze(1)
            batch.pop('label_noise_raw_lengths', None)
