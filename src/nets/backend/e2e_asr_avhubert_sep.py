...
from src.separator.tf_locoformer import tf_locoformer_separator
...
class E2E(torch.nn.Module):
    ...
    def forward(self, video, audio, video_lengths, audio_lengths, label, label_audios, label_noises, label_audio_lengths, label_noise_lengths):
        ...
        if out_s.shape[0] > B:
            repeat_times = out_s.shape[0] // B - 1
            loss_s_rest = self.mse_loss_feat(out_s[:-B,:,:], label_audios.repeat(repeat_times, 1, 1))
            loss_sep = loss_s_main + loss_s_rest * 0.1
        else:
            loss_sep = loss_s_main
