...
class AVSRCocktailModel(BaseInferenceModel):
    ...
    def load_model(self):
        ...
        # Inference doesn't use SEANet; drop it before moving to GPU to save VRAM
        if hasattr(avsr_model.avsr, "separator"):
            del avsr_model.avsr.separator
...
class InferenceEngine:
    ...
    def infer_video(self, video_path, asd_path=None, offset=0., desc=None):
        ...
        for seg in tqdm(segments, desc="Processing segments" if desc is None else desc, total=len(segments)):
            ...
            sample = {
                "video": video_path,
                # "audio": video_path.replace('.mp4', '_clean_curr_4spk_offset_4s.wav'),
                "start_time": seg[0],
                "end_time": seg[1],
            }
