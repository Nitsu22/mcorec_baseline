...
class AVSRCocktailModel(BaseInferenceModel):
    ...
    def load_model(self):
        from src.dataset.avhubert_dataset_face import AudioTransform, VideoTransform, DataCollator
    ...
    def mcorec_session_infer(self, session_dir, output_dir):
        ...
        for speaker_name, speaker_data in tqdm(metadata.items(), desc="Processing speakers", total=len(metadata)):
            ...
            for idx, track in enumerate(speaker_data['central']['crops']):
                # Use track['video'] (track_XX.mp4, 224x224 face crop) instead of track['lip'] (track_XX_lip.av.mp4, 96x96 mouth crop)
                video_path = os.path.join(session_dir, track['video'])
