...
def load_avsr_dataset(cache_dir='data-bin/cache', include_mcorec=True, streaming=False):
    ...
    while not finished_loading:
        try:
            ...
            if include_mcorec:
                ...
                warnings.warn("include_mcorec=False is not supported in this script. Please use --include_mcorec flag.", UserWarning)
                raise ValueError("This script only supports MCoRec dataset. Please use --include_mcorec flag.")
    ...
    if include_mcorec:
        # MCoRec 100% for training
        map_datasets = {
            "mcorec": {
                "probabilities": 1.0,
                "dataset": {
                    "train": mcorec_dataset["train"],
                    "valid": mcorec_dataset["valid"],
                }
            }
        }
    else:
    ...
    train_datasets_list = [item['dataset']['train'] for item in map_datasets.values() if item['dataset']['train'] is not None]
    if len(train_datasets_list) > 1:
        train_dataset = datasets.interleave_datasets(train_datasets_list, 
                                                     seed=11,
                                                     probabilities=[item['probabilities'] for item in map_datasets.values() if item['dataset']['train'] is not None], 
                                                     stopping_strategy='all_exhausted')
    elif len(train_datasets_list) == 1:
        train_dataset = train_datasets_list[0]
    else:
        raise ValueError("No training dataset available")
    
    valid_datasets_list = [item['dataset']['valid'] for item in map_datasets.values() if item['dataset']['valid'] is not None]
    if len(valid_datasets_list) > 1:
        valid_dataset = datasets.interleave_datasets(valid_datasets_list,
                                                     stopping_strategy='first_exhausted')
    elif len(valid_datasets_list) == 1:
        valid_dataset = valid_datasets_list[0]
    else:
        valid_dataset = None
    ...
    # load lrs2 for interference speech (only if not using MCoRec only)
    if include_mcorec:
        # For MCoRec only, interference speech is not needed
